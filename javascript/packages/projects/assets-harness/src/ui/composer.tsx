...
import { agentInfo, effortOf, modelOf } from "../../shared/agents.js";
...
function Composer({ state }: { state: TabState }): React.JSX.Element {
  ...
  const text = draftsByTab.value[tabId] ?? "";
  ...
  const model = modelOf(state.tab);
  const effort = effortOf(state.tab);
          ...
          <select
            ...
            title={
              started
                ...
                : "The model this file's runs use. The first one until you pick another."
            }
          ...
          <select
            ...
            title={
              started
                ...
                : "Reasoning effort for this file. High until you pick another."
            }
            disabled={started || state.running}
            onChange={(event) => {
