...
const assets = signal<AssetIndex | null>(null);
...
/**
 * The text typed into each tab's composer and not sent yet. It lives here, not
 * in the composer: the workspace is mounted afresh for every tab, so a draft
 * kept in the component would be gone after a switch to another tab and back.
 */
...
const showProjects = signal(false);
/** The Assets panel on the left of the window. */
const showAssets = signal(true);
...
function setDraft(tabId: string, text: string): void {
  draftsByTab.value = { ...draftsByTab.value, [tabId]: text };
}
...
function onEvent(event: WorkspaceEvent): void {
  if (event.type === "project") {
    ...
    if (event.project === null || assets.value?.projectPath !== event.project.path) {
      assets.value = null;
    }
    return;
  }
  if (event.type === "assets") {
    if (event.index.projectPath === project.value?.path) {
      assets.value = event.index;
    }
    return;
  }
  if (event.type === "tab") {
...
/** Opens a project folder. The main process closes the tabs of the old one and pushes the new ones. */
async function openProject(projectPath: string): Promise<boolean> {
  try {
    project.value = await harness.projects.open(projectPath);
    assets.value = await harness.projects.assets();
    recentProjects.value = await harness.projects.recent();
...
async function pickProject(): Promise<void> {
  try {
    const picked = await harness.projects.pick();
    if (picked) {
...
async function rescanAssets(): Promise<void> {
  try {
    assets.value = await harness.projects.rescan();
  } catch (error) {
    fail(error);
  }
}

function revealAsset(relPath: string): void {
  harness.projects.reveal(relPath).catch(fail);
}

/** Brings a model of the Assets panel up: its tab when one is open, a new tab otherwise. */
async function openModel(blendPath: string): Promise<void> {
  const open = tabs.value.find((state) => state.tab.blendPath === blendPath);
  if (open) {
    await selectTab(open.tab.id);
    return;
  }
  await createTab(blendPath);
}

// ---------------------------------------------------------------------------
// Tabs.

async function createTab(nameOrPath: string): Promise<boolean> {
  try {
    const state = await harness.tabs.create(nameOrPath);
    upsertTab(state);
