...
import { MCP_SERVER_NAME, promptText, shorten, summariseCode } from "./driver.js";
 ...
 *
 * Codex runs in the project folder and reads the project's instruction files
 * on its way from the repository root down to that folder. A project written
 * for Claude Code has `CLAUDE.md` files and no `AGENTS.md`, so both names are
 * read.
 */
...
async function stageImages(images: TurnImage[]): Promise<{ paths: string[]; release: () => Promise<void> }> {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), "assets-harness-turn-"));
  const paths: string[] = [];
...
async function run(request: TurnRequest, out: TurnReport): Promise<void> {
  // The working directory is the project folder. The harness writes nothing
  // into it: its instructions ride the first message, not an AGENTS.md.
  ...
  const codex = new Codex({
    ...
    env,
    config: {
      // The project's conventions are what the run is for, so its instruction
      // files are read. A project set up for Claude Code names them CLAUDE.md.
  ...
  const options: ThreadOptions = {
    ...
    // reaches the network and may write — into the project folder only.
