 ...
 *
 * Claude runs in the project folder and reads the project's settings only:
 * the `CLAUDE.md` files from the folder up to its repository root, the
 * project's `.claude/settings.json` and its skills. The user's own global
...
/**
 * What a run is allowed to do without being asked.
 *
 * A run writes nothing outside the project folder: the textures it downloads
 * go to `Assets/textures/<Model>/`, the exports to `Assets/export/`, and the
 * `.blend` and the renders are written by Blender, not by these tools. So
 * `Write` and `Edit` are scoped to the project, and Bash runs inside the OS
 * sandbox below, which has the same bound.
 */
const ALLOWED = [
...
async function run(request: TurnRequest, out: TurnReport): Promise<void> {
  ...
  const options: Options = {
    ...
    // The project shapes the run, the user's global setup does not.
