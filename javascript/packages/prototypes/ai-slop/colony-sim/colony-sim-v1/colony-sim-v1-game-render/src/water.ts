...
import { type Grid, inBounds, Terrain, tileIndex } from "@hw/colony-sim-v1-core";
...
function touchesLand(grid: Grid, x: number, y: number): boolean {
  return touches(grid, x, y, Terrain.Grass)
    || touches(grid, x, y, Terrain.Rock)
    || touches(grid, x, y, Terrain.Mountain);
}
