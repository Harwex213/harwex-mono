...
// What the cursor is over, as resolved by the engine — the view knows tiles, not
// what lives on them.
type HoverKind = "entity" | "tile" | null;

// Pointer events the view reports outwards. Picking needs the World, which the
// view does not have, so the engine decides what a tile coordinate means.
...
class Camera {
  ...
  constructor(app: Application, root: Container, commands: CommandDispatcher, handlers: PointerHandlers) {
    this.app = app;
    this.root = root;
...
export type { HoverKind, PointerHandlers };
