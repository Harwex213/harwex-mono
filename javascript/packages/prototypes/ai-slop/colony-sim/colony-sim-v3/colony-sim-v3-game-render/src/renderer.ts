...
import { ShadowPool } from "./shadows";
...
class GameRenderer implements GameView {
  ...
  // Null when the pass is off: a flag the renderer reads once at construction and
  // never again, rather than a branch in every place a shadow could appear.
  private shadows: ShadowPool | null;
  ...
  constructor(
    ...
    this.shadows = flags.shadows ? new ShadowPool(app.renderer, this.layers.shadows) : null;
    ...
    const ground = buildGround(world, flags);
  ...
  render(world: World, alpha: number): void {
    ...
    for (const [id, sprite] of this.sprites) {
      ...
      sprite.zIndex = y;
      // The shadow follows the interpolated position too, not the tile: it belongs
      // to the same walk as the sprite it is under.
      this.shadows?.move(id, sprite.x, sprite.y);
    }
  ...
  private reconcile(world: World): void {
    ...
    for (const [id, sprite] of this.sprites) {
      if (!world.entities.has(id)) {
        ...
        this.shadows?.detach(id);
      }
    }
  }

  // Everything on the map comes from the 16px sheets; which one is decided by the
  // component that makes the entity what it is. The same branch says how big a
  // shadow it drops: nothing else in the renderer knows what kind of thing this is.
  private createSprite(world: World, id: EntityId): void {
    if (world.animals.has(id)) {
      this.shadows?.attach(id, "chicken");
    ...
    if (world.rocks.has(id)) {
      ...
      this.layers.objects.addChild(sprite);
      this.sprites.set(id, sprite);
      this.shadows?.attach(id, "rock");
      return;
    }

    this.shadows?.attach(id, "colonist");
