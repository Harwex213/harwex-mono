...
import { createRng, type Grid, Terrain, TILE_SIZE, tileIndex, type World } from "@hw/colony-sim-v3-core";
...
function materialMap(world: World): Uint8Array {
  ...
  for (let y = 0; y < grid.height; y += 1) {
    for (let x = 0; x < grid.width; x += 1) {
      ...
      // shade and tuft picks apply. A mountain is not its own material: the cliff
      // nine-slice is laid on top of the bake and its round frames leave the
      // corners transparent, so what a mountain needs from this layer is the
      // barren ground its mass rises out of.
      if (terrain === Terrain.Rock || terrain === Terrain.Mountain) {
