/**
 * `assets/hammer.svg`, inlined.
 *
 * The rspack config carries no asset rule and no SVG loader, and adding one for
 * a single 500-byte icon would buy a network round trip and a loader dependency
 * for nothing. `fill="currentColor"` is kept, so the icon takes the button's
 * colour and the pressed state needs no second file.
 */
function HammerIcon(): React.JSX.Element {
  return (
    <svg viewBox="0 0 576 512" role="img" aria-hidden="true" focusable="false">
      <path
        fill="currentColor"
        d="m571.31 193.94l-22.63-22.63c-6.25-6.25-16.38-6.25-22.63 0l-11.31 11.31l-28.9-28.9c5.63-21.31.36-44.9-16.35-61.61l-45.25-45.25c-62.48-62.48-163.79-62.48-226.28 0l90.51 45.25v18.75c0 16.97 6.74 33.25 18.75 45.25l49.14 49.14c16.71 16.71 40.3 21.98 61.61 16.35l28.9 28.9l-11.31 11.31c-6.25 6.25-6.25 16.38 0 22.63l22.63 22.63c6.25 6.25 16.38 6.25 22.63 0l90.51-90.51c6.23-6.24 6.23-16.37-.02-22.62m-286.72-15.2c-3.7-3.7-6.84-7.79-9.85-11.95L19.64 404.96c-25.57 23.88-26.26 64.19-1.53 88.93s65.05 24.05 88.93-1.53l238.13-255.07c-3.96-2.91-7.9-5.87-11.44-9.41z"
      />
    </svg>
  );
}

export { HammerIcon };
