...
import { HEX_DIRECTIONS } from "@hw/ostrov-utils";
import type { TAxial } from "@hw/ostrov-utils";

/**
 * Outline of a coast. Every hex edge with water on the far side becomes one
 * segment, so the shared edges inside the landmass are left out. Corner `i` and
