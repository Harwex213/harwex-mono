...
/** Everything the caller can turn. The seed decides the layout, the rest the mix. */
type TIslandOptions = {
  seedText: string;
  /** Radius of the local grid the island is grown on. */
  radius: number;
 ...
 *
 * The shares come from the island type, so the mix is what was asked for while
 * the seed still decides where each band lands.
 */
const assignTerrain = (
  ...
  // Half the ranking is height and half is how far the cell sits from the water,
  // which keeps the range in the middle of the island instead of on the beach.
  const peakScore = (key: string) => {
    const inland = deepest > 1 ? (depth.get(key)! - 1) / (deepest - 1) : 0;

    return elevation.get(key)! * 0.55 + inland * 0.45;
  };
...
const generateIsland = ({ seedText, landCount, terrainWeights, radius }: TIslandOptions): TIsland => {
  ...
  const land = pickLand(byKey, elevation, landCount);
  ...
  for (const cell of grid) {
    ...
    const bonus = coastal.has(key) ? 0.15 : 0;

    moisture.set(key, clamp(noise.get(key)! * 0.85 + bonus, 0, 1));
  }

  const terrain = assignTerrain([...land], elevation, moisture, depth, terrainWeights);
...
/** A fresh seed for the "another island" button. */
const createSeedText = () => Math.floor(Math.random() * 0xffffff).toString(36).toUpperCase();

export type { TIsland, TIslandOptions, TTerrainWeights, TTile };
export { createSeedText, generateIsland };
