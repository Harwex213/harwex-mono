...
/** Centre of an offset cell in pixel space. */
const offsetToPixel = (col: number, row: number, size: number): TPoint => ({
  x: hexWidth(size) * (col + 0.5 * (row & 1) + 0.5),
  y: 1.5 * size * row + size,
});

/** Inverse of `offsetToPixel`, rounded to the containing cell. */
...
export type { TPoint };
export { HEX_CORNER_ANGLES, hexWidth, mapPixelSize, offsetToPixel, pixelToOffset };
